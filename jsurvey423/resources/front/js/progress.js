var projsurveyid = $("#dczjid").val();

var json = $("#jsonquesid").val();
json = '[' + json + ']';
var jsonquesid = JSON.parse(json);
var num = 0;
var pageNum = 0;
var pageNumCount = 0;
for(var ever in jsonquesid) {  
	num++;  
}  
var quesJson;
...
$.ajax({
	 type: "post",
	 url:contextpath+"/front/dczj/findQuesAndAnswJson.do",
	 data:"dczjid="+projsurveyid,
	 success:function(result){
	 	if(result.success){
            var hiddenQuesJson = result.params.hiddenQuesJson;
            quesJson = JSON.parse(hiddenQuesJson);
            setTimeout('hiddenques(quesJson);',10);
	 	}
	}
});

function hiddenques(quesJson){
...
$.ajax({
	 type: "post",
	 url:contextpath+"/front/dczj/findPageNum.do",
	 data:"dczjid="+projsurveyid,
	 success:function(result){
	 	if(result.success){
	 		pageNumCount = result.params.pageNum;
	 		setTimeout('refreshcss();',10);
	 	}else{
...
function refreshcss(){
	var html = "<div style='text-align: center;position:relative;height: 36px;'><div id='btnNext' onclick='show_next_page();' >下一页</div>";
	$("#dczj"+projsurveyid).append(html);
...
$.ajax({
	 type: "post",
	 url:contextpath+"/front/dczj/findValidateQues.do",
	 data:"dczjid="+projsurveyid,
	 success:function(result){
	 	if(result.success){
...
function validateQues(){
	var mustfillquesid = sessionStorage.getItem("mustfillquesid_"+projsurveyid);
	var mustfilltype = sessionStorage.getItem("mustfilltype_"+projsurveyid);
	if(mustfillquesid >0){
		if(mustfilltype == 0){
			 ...
			 if(radioval != null && radioval != undefined){
				 ...
				 sessionStorage.removeItem("mustfillquesid_"+projsurveyid);
				 sessionStorage.removeItem("mustfilltype_"+projsurveyid);
			 }
		}else if(mustfilltype == 1){
			 var checkboxnum = 0 ;
	    	 var checkboxname = "surveyAnsw"+ mustfillquesid;
			 var checkboxName=document.getElementsByName(checkboxname);
			 var checkboxminvalue = sessionStorage.getItem("mustfillmin_"+projsurveyid);
			 var checkboxmaxvalue = sessionStorage.getItem("mustfillmax_"+projsurveyid);
			 for(var kkk=0;kkk<checkboxName.length;kkk++){
			  ...
			  if(checkboxnum >= checkboxminvalue && checkboxnum <= checkboxmaxvalue){
				  ...
				  sessionStorage.removeItem("mustfillquesid_"+projsurveyid);
				  sessionStorage.removeItem("mustfilltype_"+projsurveyid);
				  sessionStorage.removeItem("mustfillmin_"+projsurveyid);
				  sessionStorage.removeItem("mustfillmax_"+projsurveyid);
			  }
			...
			var textValue = $("#surveyAnsw_"+mustfillquesid+"_0").val();
			var validaterules = sessionStorage.getItem("mustfillvalidaterules_"+projsurveyid);
			textValue = $.trim(textValue)
			if(textValue.length > 0){
				if(validaterules == 0){
					sessionStorage.removeItem("mustfillquesid_"+projsurveyid);
					sessionStorage.removeItem("mustfilltype_"+projsurveyid);
					sessionStorage.removeItem("mustfillvalidaterules_"+projsurveyid);
					$('#surveyMessage'+mustfillquesid).html("");
...
function progress(){
	var count = 0;
	for ( var i in jsonquesid) {
		if (jsonquesid[i].type == 0) {
			var radioname = "surveyAnsw"+ jsonquesid[i].quesid;
			var radioval = $('input:radio[name="'+radioname+'"]:checked').val();   
			if(radioval != null){
				count++;
			}
		}else if(jsonquesid[i].type == 1){
			var checkboxques = document.getElementsByName("surveyAnsw"+ jsonquesid[i].quesid);               
            var flag = false ;               
            for(var i=0;i<checkboxques.length;i++){
                if(checkboxques[i].checked){
                    flag = true ;
                    break ;
                }
            }
		...
		} else if(jsonquesid[i].type == 2){
			var answerQuestion = $("#surveyAnsw_" + jsonquesid[i].quesid + "_0").val();
			if (answerQuestion != '' && answerQuestion.length > 0) {
				count++;
			}
		} else if(jsonquesid[i].type == 5){
			var answerQuestion = $("#surveyAnsw_" + jsonquesid[i].quesid + "_0").val();
			if (answerQuestion != '' && answerQuestion.length > 0) {
				count++;
			}
...
function progressfn(cent) {
    ...
    if(progressbar != null){
        ...
        progressbar[0].innerHTML = "答题进度 "+cent + "%";
			     ...
			     if(type == 0 && mustfill == 1){
			    	 ...
			    	 if(mustfill == 1){
			    		 if(!(textValue.length > 0 && textValue != '' && textValue != undefined)){
							  sessionStorage.setItem("mustfillquesid_"+projsurveyid,quesId);
							  sessionStorage.setItem("mustfilltype_"+projsurveyid,type);
							  sessionStorage.setItem("mustfillvalidaterules_"+projsurveyid,validaterules);
							  $('#surveyMessage'+quesId).html("（该题目必填！）");
							  location.href = "#" + tableId;
							  checknum++;
							  return;
