...
public class OprQuesController {
	...
	public JsonResult edit(Integer dczjid,Integer quesid) {
		...
		LogRecorder.record(LogEntity.getInstance().setModelName("在线调查问题").setFunctionName("编辑")
				.setIpAddr(userIp).setLogUser(currentUser.getName()+"("+currentUser.getLoginName()+")").setDescription("编辑调查表问题，调查表ID："+quesInfoEn.getDczjid()+"，问题："+quesInfoEn.getQuesname()));
					...
					for(Integer answid : answIdList){
						...
						if(bl){
							...
							LogRecorder.record(LogEntity.getInstance().setModelName("在线调查").setFunctionName("保存")
									.setIpAddr(userIp).setLogUser(currentUser.getName()+"("+currentUser.getLoginName()+")").setDescription("保存调查问题的选项，调查表ID："+surveyAnsw.getDczjid()+"，题目ID："+surveyAnsw.getQuesid()+"，答案ID："+surveyAnsw.getIid()+"，答案："+surveyAnsw.getAnswname()));
						}
					}
				}
			}
