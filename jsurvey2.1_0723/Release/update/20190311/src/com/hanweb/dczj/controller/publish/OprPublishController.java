...
public class OprPublishController {
	...
	public ModelAndView doPreview(String dczjid,int pcstyle,int webid) {
		...
		if(pcstyle == 1) {
			strContent = publishService.startingPage(settingEn,styleEn,dczjid,confMap,displayConfig);
		...
		switch(state){
			...
			case 2: //已结束
				...
				if(isShow == 0){
					...
					int ishtml =1;
					strContent = publishService.getResultPage(settingEn,styleEn,titleInfo,confMap,displayConfig,ishtml);
