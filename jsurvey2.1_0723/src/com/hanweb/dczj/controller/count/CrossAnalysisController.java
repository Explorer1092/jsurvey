...
public class CrossAnalysisController {
	...
	public ModelAndView showCrossAnalysisPage(String dczjid) {
		...
		List<QuesInfo> quesList1 = quesInfoService.findRadioQuesByDczjId1(dczjid);
		TitleInfo title = titleInfoService.getEntity(NumberUtil.getInt(dczjid));
		if(title!=null) {
		  if(title.getType() == 0) {
			  quesList1 = quesInfoService.findRadioQuesByDczjId(dczjid); 
		  }
		}
		modelAndView.addObject("dczjid", dczjid);
		modelAndView.addObject("quesList", quesList);
		modelAndView.addObject("quesList1", quesList1);
		return modelAndView;
	}
	...
	public JsonResult oprCrossAnalysis(String xquesid, String yquesid) {
		...
		if(xAnswInfoList != null && xAnswInfoList.size() > 0) {
			for(AnswInfo answInfoEn : xAnswInfoList) {
				...
				if(StringUtil.equals(answInfoEn.getIsright(), "1")) {
					if(answnames.indexOf("<img") != -1) {
						answnames = StringUtil.removeHTML(answnames)+"[图片]";
					}
					answnames = StringUtil.removeHTML(answnames);
		...
		if(yAnswInfoList != null && yAnswInfoList.size() > 0) {
			for(AnswInfo yanswInfoEn : yAnswInfoList) {
				...
				yAxis_data += "'" + StringUtil.removeHTML(answnames) + "',";
				if(xAnswInfoList != null && xAnswInfoList.size() > 0) {
		...
		if(xAnswInfoList != null && xAnswInfoList.size() > 0) {
			for(AnswInfo xanswInfoEn : xAnswInfoList) {
				int xanswid = xanswInfoEn.getIid();  //x轴
				String seriesDatas ="";
				String answnames = xanswInfoEn.getAnswname();
				if(answnames.indexOf("<img") != -1) {
					answnames = StringUtil.removeHTML(answnames)+"[图片]";
				}
				answnames = StringUtil.removeHTML(answnames);
				seriesData += " { name: '"+answnames+"',type: 'bar', barMaxWidth: 30,stack: '总量',label: {normal: { show: false,position: 'insideRight' }},data: [";				
				if(yAnswInfoList != null && yAnswInfoList.size() > 0) {
					for(AnswInfo yanswInfoEn : yAnswInfoList) {
