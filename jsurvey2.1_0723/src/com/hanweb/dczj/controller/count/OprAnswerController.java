...
public class OprAnswerController {
	...
	public JsonResult upopen(String ids) {
		JsonResult jsonResult = JsonResult.getInstance();
		String[] idArray = ids.split(",");
		String contentid = "";
		try {
			for(int i=0;i<idArray.length;i++) {
				contentid = idArray[i];
				contentRecoService.updateAllAudiById1(contentid);
			}
			jsonResult.setSuccess(true);
		} catch (Exception e) {
			jsonResult.setSuccess(false).setMessage("操作失败，请联系系统管理员");
	 ...
	 */
	@ResponseBody
	@RequestMapping("saveopenly")
	public JsonResult saveopenly(String contentid, String replycontent) {
		JsonResult jsonResult = JsonResult.getInstance();
		try {
			contentRecoService.saveOpenly(contentid, replycontent);
			jsonResult.setSuccess(true);
		} catch (Exception e) {
			jsonResult.setSuccess(false).setMessage("操作失败，请联系系统管理员");
		}
	...
	public ModelAndView exportansandrep(String dczjid,String answid,String quesid,String starttime,String endtime,String ids) {
//		File file = null;
...
//		}
		...
		String fileName = exportdetailStr(dczjid, answid, quesid, starttime, endtime,ids);
	...
	public String exportReportStr(String dczjid, String starttime, String endtime) {
		...
		for(QuesInfo quesInfo : quesInfos) {
			if(quesInfo.getType()==2 || quesInfo.getType()==5) {
				...
				String quesname = quesInfo.getQuesname();
				...
				rows.add(valueList);
			}else if(quesInfo.getType()==0 || quesInfo.getType()==1 || quesInfo.getType()==6 || quesInfo.getType()==7) {
				sum = 0;
				/*问题*/
				valueList = new ArrayList<String>();
				String quesname = quesInfo.getQuesname();
					...
					for(AnswInfo answInfo : answInfos) {
						...
						if(quesInfo.getType()==6 || quesInfo.getType()==7) {
							if(StringUtil.equals(answInfo.getIsright(), "1")) {
								 answname = answname + "（正确答案）";
							}	
						}
						
						valueList.add(answname);
	...
	public String exportdetailStr(String dczjid,String answid,String quesid,String starttime,String endtime,String ids) {
		String filePath = "";
		List<List<String>> rows = new ArrayList<List<String>>();
		List<String> headList = new ArrayList<String>();/* 表头 */
		List<String> valueList = null; /* 数据列 */
...
//		int count = contentRecoService.findEntityCountByTime1(dczjid, quesid, answid, starttime, endtime,ids);
		page = count/1000 + 1;
		for(int i=1;i<=page;i++) {
//			contentRecos.addAll(contentRecoService.findEntityListByTime(i+"", 1000+"", dczjid, quesid, answid, starttime, endtime));
			contentRecos.addAll(contentRecoService.findEntityListByTime1(i+"", 1000+"", dczjid, quesid, answid, starttime, endtime,ids));
		...
		for(ContentReco contentReco : contentRecos) {
			valueList = new ArrayList<String>();
			String answcontent = contentReco.getAnswcontent();
			if(answcontent.indexOf("<img") != -1) {
				answcontent = StringUtil.removeHTML(answcontent)+"[图片]";
			}
			answcontent = StringUtil.removeHTML(answcontent).replace("&nbsp;"," ");
			valueList.add(answcontent);
			...
			valueList.add(type);
			String replycontent = contentReco.getReplycontent();
			if(replycontent==null||replycontent.isEmpty()) {
				replycontent="无";
			}
			if(replycontent.indexOf("<img") != -1) {
				replycontent = StringUtil.removeHTML(replycontent)+"[图片]";
			}
			replycontent = StringUtil.removeHTML(replycontent).replace("&nbsp;"," ");
