...
public class OprTitleController {
	...
	public JsonResult submitAdd(Integer webid,String titlename,int type,String username) {
		...
		List<TitleInfo> titleInfos=titleInfoService.findTitleByWebIdAndType(webid,type);
		...
		if(titleInfos!=null && titleInfos.size()>0) {
			for (TitleInfo titleInfo : titleInfos) {
				if(titlename.equals(titleInfo.getTitlename())) {
					jsonResult.setSuccess(false);
					jsonResult.setMessage("同一类型标题不能相同");
					return jsonResult;
				}	
			}
		}
...
//			nOrderID = NumberUtil.getInt(strData[0][0]);
	...
	@ResponseBody
	public JsonResult totop(int webid,int type,int dczjId) {
		JsonResult jsonResult = JsonResult.getInstance();
		jsonResult.setSuccess(false);
		List<Integer> dczjIdList = titleInfoService.findDczjIdByWebIdAndType(webid,type);
		int k=0;
			...
			for(int i = 0; i<= dczjIdList.size() - 1; i++){
				if(dczjIdList.get(i) == dczjId){
						if(i > 0){
							...
							k++;
							if(k<=i) {
								int sortDczjId = dczjIdList.get(i-k);
								dczjIdList.set(i-k+1, sortDczjId);
								dczjIdList.set(i-k, dczjId);
								}
							}
						}else{
							return jsonResult.setMessage("已经置顶！");
						}
					}
				}
		}
