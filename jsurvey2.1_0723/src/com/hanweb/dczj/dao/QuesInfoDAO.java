...
public class QuesInfoDAO extends BaseJdbcDAO<Integer, QuesInfo> {
	...
	public List<QuesInfo> findTextQuesAndPageQues(String dczjid) {
		String strSql = this.getSql() + " WHERE state = 0 And (type = 0 or type = 1 or type = 2 or type = 4 or type = 5 or type = 6 or type = 7) ";
		if(StringUtil.isNotEmpty(dczjid)){
			strSql += " AND dczjid=:dczjid";
		}
		strSql +=" ORDER BY orderid DESC,iid ASC";
	...
	public int selectnum(Integer dczjid) {
		String sql="SELECT count(1) FROM " + Tables.DCZJQUESINFO + " where dczjid = :dczjid and (type =6 or type=7) and state = 0";
		Query query = this.createQuery(sql);
		query.addParameter("dczjid", dczjid);
		return this.queryForInteger(query);	
	}
	public int selectscore(Integer dczjid) {
		String sql="SELECT SUM(quesscore) FROM " + Tables.DCZJQUESINFO + " where dczjid = :dczjid and (type =6 or type=7) and state = 0";
		Query query = this.createQuery(sql);
		query.addParameter("dczjid", dczjid);
		return this.queryForInteger(query);	
	}
	public List<QuesInfo> findRadioQuesByDczjId(String dczjid) {
		String strSql = this.getSql() + " WHERE state = 0 And type = 0 AND dczjid=:dczjid ORDER BY orderid DESC,iid ASC";
		Query query = this.createQuery(strSql);
		query.addParameter("dczjid", dczjid);
		return this.queryForEntities(query);
	}
	...
	public List<QuesInfo> findRadioQuesByDczjId1(String dczjid) {
		String strSql = this.getSql() + " WHERE state = 0 And type = 6 AND dczjid=:dczjid ORDER BY orderid DESC,iid ASC";
		Query query = this.createQuery(strSql);
		query.addParameter("dczjid", dczjid);
		return this.queryForEntities(query);
	}
