...
public class StyleService {
	 ...
	 * @return
	 */
	public String selectInfoQues(String strContent) {
		String tmqs = "<!--unitname=下拉区-->";
		String tmqe = "<!--/unitname=下拉区-->";
		int begin = strContent.indexOf(tmqs)+19;
		int end = strContent.indexOf(tmqe);
		strContent = strContent.substring(begin, end);
		return strContent;
	...
	public String selectInputQues(String strContent){
		String tmqs = "<!--unitname=下拉题-->";
		String tmqe = "<!--/unitname=下拉题-->";
		int begin = strContent.indexOf(tmqs)+19;
		int end = strContent.indexOf(tmqe);
		strContent = strContent.substring(begin, end);
		return strContent;
		...
		if(m1.find()){  
			...
			while(it.hasNext()){ 
				...
				if(quesType == 8){
					...
					String content = "";
					styleCode = this.getStyleCodeByPath(quespage);
					content = this.selectInfoQues(styleCode);
...
//							option += "<option value ='"+answInfo.getAnswname()+"' selected >"+answInfo.getAnswname()+"</option>";
//						}else {
							option += "<option value ='"+answInfo.getAnswname()+"' >"+answInfo.getAnswname()+"</option>";
//						}
						
					}
					content = content.replaceAll("<!--quesname-->", quesname)
							         .replaceAll("<!--divquesId-->", quesId+"")
	...
	public String parseQues(String content, int quesId) {
		...
		if(m1.find()){  
			...
			for(AnswInfo en : answList){
				...
				String defaultvalue = "";
				if (NumberUtil.getInt(en.getDefaultvalue())==1) {
