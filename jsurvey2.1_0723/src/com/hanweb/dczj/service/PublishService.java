...
public class PublishService {
	...
	public String startingPage(Dczj_Setting settingEn, Style styleEn, String dczjid,
			HashMap<String, Object> confMap, DisplayConfig displayConfig,String index) {
		String temp = null;
		...
		String strContent = this.parseDczjFront(dczjid,confMap,settingEn,style,displayConfig,index);
	...
	public String parseDczjFront(String dczjid, HashMap<String, Object> confMap, Dczj_Setting settingEn,
			String style, DisplayConfig displayConfig,String index) {
		...
		if(quesList == null){
			...
			//解析列表题目
			strContent += parseFormFront(confMap,quesList,settingEn,index);
		}
		return strContent;
	}
     ...
     * @return
     */
	private String parseFormFront(HashMap<String, Object> confMap, List<QuesInfo> quesList, Dczj_Setting settingEn,String index) {
					...
					if(pageType ==0 && quesType == 5){	//单行文本
						...
						if(userinfo != null && userinfo.length()>0){
							userinfo = "<table id=\"surveyTable"+quesId+"\" class=\"jsurvey-table\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\"><tr>"
						             ...
						             + "<td class=\"jsurvey-td-input\">"
						             + "<input type=\"text\" id='surveyAnsw_" + quesId + "_" + 0 + "' name='surveyAnsw" + quesId + "_content' class='jsurvey-td-info-input'/>"
						             		+ "<span id=\"surveyMessage"+quesId+"\" class=\"jsurvey-messagebox\"></span></td><td></td></tr></table>";
						}
						strInner += userinfo;  //题目类型为小节时。获取题目内容
						continue;
					}
					if(pageType ==0 && quesType == 8){	//下拉题
						String userinfo = StringUtil.getString(quesEn.getQuesname());
						String selectstring = " <select name='surveyAnsw"+quesEn.getIid()+"'><option value =''>-请选择-</option> ";
						List<AnswInfo> answIdList = answInfoService.getAnswListByQuesId(quesEn.getIid());
						for (AnswInfo answInfo : answIdList) {
							if (NumberUtil.getInt(answInfo.getDefaultvalue())==1) {
								selectstring += "<option value ='"+answInfo.getIid()+"' selected>"+answInfo.getAnswname()+"</option>";
							}else {
								selectstring += "<option value ='"+answInfo.getIid()+"'>"+answInfo.getAnswname()+"</option>";
							}
						...
						if(userinfo != null && userinfo.length()>0){
							userinfo = "<table id=\"surveyTable"+quesId+"\" class=\"jsurvey-table\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\"><tr>"
						             + "<td class=\"jsurvey-td-info\"><span class=\"jsurvey-td-info-span\">"+userinfo+"</span></td>"
						             + "<td class=\"jsurvey-td-input\">" + selectstring
						             + "</td><td></td></tr></table>";
						}
						strInner += userinfo;  //题目类型为小节时。获取题目内容
						continue;
...
*/
				...
				strFor = styleService.parseList(2,quesConfMap,answList,index); //解析答案列表
				HashMap<String,Object> tag = styleService.setTag(1,quesEn,confMap,false, k,index);//解析题目样式中的标签
...
//		confMap.put("userbasemessage", userbasemessage);
		if(pageType==0){//表单页
			...
			strContent = "<input type='hidden' name='dczjid' id='dczjid' value='"+dczjid+"'>"+proContent+"<form id=\"" + formName + "\" name=\"" + formName + 
						...
						"\">" + strContent + "<input type='hidden' name='submitType' value='1'><input type='hidden' name='isprize' id='isprize' value='"+settingEn.getIsprize()+"'><input type='hidden' name='sumscore' id='sumscore' val=''><input type='hidden' name='rightnum' id='rightnum' val=''><input id='cookie"+dczjid+"' name='cookieId' type='hidden' value=''></form>";
	...
	public String getResultPage(Dczj_Setting settingEn, Style styleEn, TitleInfo titleInfo, HashMap<String, Object> confMap,
			DisplayConfig displayConfig, int ishtml,String index) {
		String temp = null;
		...
		if(style == null || style.length() == 0){ //如果没设置样式时，，设置默认样式
			...
			if(titleInfo.getType() == 3) {
			  dauatlpage = BaseInfo.getRealPath()+ "/resources/dczj/template/result1.xml";
			}		
			style = styleService.getStyleCodeByPath(dauatlpage);
			if(titleInfo.getType() == 3) {
				style = style.replace("<!--dczjtitle-->",titleInfo.getTitlename());
			}
		}
		style = style.substring(1,style.length()-1);
		String cssstyle = displayConfig.getCssstyle();
		...
		strContent += this.parseFormFront(confMap,quesList,settingEn,index);
