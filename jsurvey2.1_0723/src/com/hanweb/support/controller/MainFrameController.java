...
public class MainFrameController {
	...
	public ModelAndView showQuesBank() {
		ModelAndView mav = new ModelAndView("dczj/mainpage/homepage");
		CurrentUser user = UserSessionInfo.getCurrentUser();
		String rangeName = user.getRangeName();
		List<Role> currentRoles = user.getRoleList();
		rangeName = "";
		...
		for (Role role : currentRoles) {
			...
			case 1: //系统管理员
				...
				tree.addNode(TreeNode.getInstance(0 + "", "", nodeName));
				for (DCZJ_WebSite web : webList) {
					tree.addNode(TreeNode.getInstance(web.getIid() + "", 0 + "", web.getName(),"/manager/dczj/quesbanktitle.do", web.getIsParent(), false));
				}
				surveynum++;
				isSysManager=1;
				break;
			case 2://网站管理员
				if(surveynum == 0){
					...
					for (Integer webid : webIdsList) {
						webName = webSiteService.findWebNameByWebId(webid);
						tree.addNode(TreeNode.getInstance(webid + "", 0 + "", webName,"/manager/dczj/quesbanktitle.do", false, false));
					}
				}
				ishave = true;// 判断树是否已经生成
				break;
