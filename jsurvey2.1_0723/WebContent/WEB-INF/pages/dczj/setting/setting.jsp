...
<html>
<head>
<h:head pagetype="page" calendar="true" toggle="true" multiselect="true" ></h:head>
		...
		<table>
			...
			<tr height="40px;">
				...
				<td>
					...
					onClick="WdatePicker({dateFmt:'yyyy-MM-dd',onpicked:function(){var endDate = $('#endtime').val();var startdate = $('#starttime').val();modify('starttime',startdate,1);modifyStateByTime(startdate, endDate);},maxDate:'#F{$dp.$D(\'endtime\')}'})" readonly="readonly">
				</td>
			</tr>
			<tr height="40px;">
			...
			<tr height="40px;">
				...
				<input type="hidden" value="${setting.islimituser}" id="islimituser" name="islimituser" />
				</td>
			...
			<tr class="islimituser_hide" height="20px;"><td></td>
				<td style="font-size: 12px;color: #999999">
				    <input type="checkbox" name="like[write]" title="前台用户">（默认全部）
				    <input type="checkbox" name="like[read]" title="后台用户" checked>（可<a onclick="openDialog('${contextPath}/manager/setting/limit_user.do?dczjid=${dczjid}',600,500,{title:'指定后台登录用户'})" style="text-decoration: underline;color: #01AAED;cursor: pointer;">指定对象</a>）
				</td>
			</tr>
<%-- 			<tr class="islimituser_hide"><td></td><td><a onclick="openDialog('${contextPath}/manager/setting/limit_user.do?dczjid=${dczjid}',600,500,{title:'指定登录用户'})" style="text-decoration: underline;color: #01AAED;cursor: pointer;">指定后台登录用户</a></td></tr> --%>
		</table>
	</div>
